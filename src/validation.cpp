                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s bitcoinvip additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.nHeight, block.GetHash(), block.nBits,  consensusParams))
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
	...
	*/
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        // GetTransactionSigOpCost counts 3 types of sigops:
					...
					if (fSpentIndex) {
						...
						spentIndex.push_back(std::make_pair(CSpentIndexKey(input.prevout.hash, input.prevout.n), CSpentIndexValue(txhash, j, pindex->nHeight, coin.out.nValue, addressType, hashBytes)));
					}
				}
    ...
    if (!WriteTxIndexDataForBlock(block, state, pindex, addressIndex, addressUnspentIndex, spentIndex))
        return false;
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.nHeight, block.GetHash(), block.nBits,  consensusParams))
